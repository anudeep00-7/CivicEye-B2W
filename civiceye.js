...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function displayAdminIssues() {
        ...
        const issues = [...allReports].sort((a, b) => {
            // Handle both numeric and string IDs for sorting
        ...
        container.innerHTML = issues.map(issue => {
            ...
            <div class="issue-card">
                ...
                <div class="issue-meta">
                    ...
                    ${issue.assignedToNgo ? `<span><i class="fas fa-hands-helping"></i> Assigned: ${issue.assignedToNgo}</span>` : ''}
                </div>
                <p class="issue-description">${issue.description}</p>
                ${issue.photo && issue.photo !== 'https://via.placeholder.com/150' ? `<img src="${issue.photo}" alt="Issue photo" class="issue-image">` : ''}
                <div class="admin-actions">
                    <select class="ngo-assign-select" onchange="assignToNGO('${escapedId}', this.value)">
                    ...
                    <button type="button" class="btn-danger" onclick="deleteReport('${escapedId}')"><i class="fas fa-trash"></i> Delete</button>
                    <button type="button" class="btn-warning" onclick="updateIssueStatus('${escapedId}', '${issue.status === 'resolved' ? 'pending' : 'resolved'}')">${issue.status === 'resolved' ? 'Reopen' : 'Resolve'}</button>
                    <button type="button" class="btn-info" onclick="shareOnTwitter('${escapedId}')"><i class="fab fa-twitter"></i> Share</button>
                </div>
            </div>
        `}).join('');
    }
    ...
    window.assignToNGO = async function (issueId, ngoUsername) {
        // Handle both string and numeric IDs
        const issue = allReports.find(r => String(r.id) === String(issueId));
        if (issue) {
            issue.assignedToNgo = ngoUsername;
            issue.status = ngoUsername ? 'inprogress' : 'pending';
            saveReportsToLocalStorage();
            ...
            if (db && issue.id) {
                try {
                    ...
                    if (docSnap.exists) {
                        await docRef.update({
                            assignedToNgo: ngoUsername,
                            status: ngoUsername ? 'inprogress' : 'pending'
                        });
                        console.log('Assignment synced to Firestore');
                    }
                } catch (error) {
            ...
            if (ngoUsername) {
                ...
                showNotification('Issue unassigned', 'info');
            }
            displayAdminIssues();
            updateDashboardStats();
        } else {
    ...
    window.deleteReport = async function (issueId) {
        if (!confirm('Are you sure you want to delete this report?')) return;

        // Handle both string and numeric IDs
        const issueIndex = allReports.findIndex(r => String(r.id) === String(issueId));
        if (issueIndex === -1) {
            console.error('Issue not found for ID:', issueId);
            showNotification('Issue not found', 'error');
            return;
        ...
        if (db) {
            try {
                const docRef = db.collection('issues').doc(String(issueId));
                const docSnap = await docRef.get();
                if (docSnap.exists) {
                    await docRef.delete();
                    console.log('Issue deleted from Firestore');
                }
            } catch (error) {
        ...
        allReports.splice(issueIndex, 1);
        saveReportsToLocalStorage();
            ...
            if (db && issue.id) {
                try {
                    ...
                    if (docSnap.exists) {
                        ...
                        console.log('Status synced to Firestore');
                    }
                } catch (error) {
                    console.error('Firestore sync error:', error);
                }
